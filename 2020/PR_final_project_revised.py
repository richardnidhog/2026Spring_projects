...
class Variables:
    ...
    def s_e():  # s = Susceptible    ;   e= Exposed
        ...
        >>> rate = np.random.choice(1.0 / ran_pert_dist(8, 10, 14, confidence=4, samples=1000))
        >>> inf_rate = r0 * rate
        >>> if 0.36 <= inf_rate <= 1.0:
        ...     print("Pass")
        ... else:
        ...     print("Fail")
        Pass
        ...
        # Delta R0 ≈ 5–8; outcome_rate = 1/infectious_period from PERT(8,10,14)
        ...
        infectious_rate = r0_delta * outcome_rate  # ∈ [5/14, 8/8] ≈ [0.36, 1.0]
...
def model(simulation_id: int, number_of_days: int, population: int, total_beds: int, contact_rate: float = 0.05, hospitalization_rate: float = 0.17) -> tuple:
    """
        ...
        :return: bed_count: List of available beds on a given day based on the admitted patients and outcome (recovered/dead) patients,
                    num_days: The days to be plotted on x-axis in the graph
        >>> import numpy as np; np.random.seed(0); beds, days = model(1, 2, 200, 100); len(beds) == len(days) == 2
        True
        """
    # concept of compartments - https://www.datahubbs.com/social-distancing-to-slow-the-coronavirus/
    number_of_beds = total_beds
    total_population = population
    susceptible = total_population - 1.0  # one seed case removed from S
    exposed = 0.0
    infected = 1.0                        # one index case to start the outbreak
    recovered = 0.0
    day = 0
    ...
    for i in range(number_of_days):
        ...
        # Cap each flow so no compartment is driven below zero (preserves S+E+I+R == N)
        new_exposed = min(beta * susceptible * infected / total_population, susceptible)
        new_infected = min(incubation_rate * exposed, exposed)
        ...
        day += test_result_time

        assert abs(susceptible + exposed + infected + recovered - total_population) < 1.0, (
            f"Population drift at day {i}: "
            f"{susceptible + exposed + infected + recovered:.2f} != {total_population}"
        )
...
def simulation(number_of_days: int, number_of_simulation: int, population: int, total_beds: int, do_threading=True):
    ...
    >>> overflow, beds_days, pct = simulation(2, 1, 200, 100, do_threading=False)  # doctest: +ELLIPSIS
    The Probability of vacant beds is: ...
    >>> len(beds_days) == 1
    True
    """
    count = 0
